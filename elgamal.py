...
def decrypt(key, cipher):
    ...
    for i in range(0, len(cipherArray), 2):
        # alpha = first number in pair
        alpha = int(cipherArray[i])
        # beta = second number in pair
        beta = int(cipherArray[i + 1])

        # plaintext integer = (alpha ^ -a) * beta mod p
        plain = modexp(alpha, -key.a, key.p) * beta % key.p
        # add plain to list of plaintext integers
        plaintext.append(plain)

    decryptedText = decode_str(plaintext)
